...
\begin{comdia}
  F_i \arrow[dd, "F g"'] \arrow[rrd, "\tau_i"] \\
  & & c \\
  F_j \arrow[rru, "\tau_j"']
\end{comdia}
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item $A$ をabelian group，
    ...
    \end{itemize}
    このとき，$A$ はcolimit of $F$ である．実際，任意のabelian group $G$ とgroup homomorphisms $f_H \colon H \to G$ with $f_H = f_{H'} \iota_{H, H'}$ ($\forall \iota_{H, H'} \colon H \inc H'$ in $J_A$) に対して，
    \[
    ...
    \]
    となる．これが，唯一の $\ext f \circ \iota_d = f_d$ ($\forall d \in J$) なる射であるのは明らか．よって，$c$ は colimit of $F$, where
    \begin{itemize}
